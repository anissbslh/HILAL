...
class HILAL:
    ...
    def run(self):
        ...
        results_path = os.path.splitext(self.updated_checkpoint_path)[0] + "R_ONLY_results.json"
    ...
    def _cluster_layers(self):
        """Run k-means (k=2) over 1D descriptors [r] to split sensitive vs robust."""
        if len(self.descriptors) == 0:
            raise RuntimeError("Descriptors not computed.")

        ids = sorted(self.descriptors.keys())
        # Use only r-ratio for clustering
        X = np.array(
            [[self.descriptors[i]["r"]] for i in ids],
            dtype=np.float64
        )
        ...
        # Favor robust cluster with low r-ratio (smaller r means more uniform Hessian eigenspectrum)
        # The cluster with the smaller r-ratio centroid is the robust one
        ...
        # Order robust: prefer low r-ratio only
        self.robust_ids_ordered = sorted(
            robust_ids,
            key=lambda lid: self.descriptors[lid]["r"]
        )

        logging.info(f"Sensitive ids: {self.sensitive_ids}")
        logging.info(f"Robust ids (ordered): {self.robust_ids_ordered}")

        # Save 2D clustering plot (r vs layer index)
        try:
            base = os.path.splitext(self.config.checkpoint_path)[0]
            out_path = f"{base}_cluster_r.png"

            r_values = np.array([self.descriptors[i]["r"] for i in ids], dtype=np.float64)
            ...
            plt.bar(layer_indices[mask_robust], r_values[mask_robust], 
                    color="tab:blue", label="robust", alpha=0.7, width=0.6)
            plt.bar(layer_indices[mask_sensitive], r_values[mask_sensitive], 
                    color="tab:red", label="sensitive", alpha=0.7, width=0.6)
            
            # Add layer IDs as labels
            for i, lid in enumerate(ids):
                plt.text(i, r_values[i] * 1.05, str(lid), ha='center', fontsize=8)

            plt.xlabel("Layer Index")
            plt.ylabel("r = lambda_max / trace")
            plt.title("Layer clustering based on r-ratio only")
