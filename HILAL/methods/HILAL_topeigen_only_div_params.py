...
from HILAL.trainer_evaluator import TrainerEvaluator
from HILAL.models.common.Linear import Linear as LHLinear
from HILAL.models.common.Conv2d import Conv2d as LHConv2d
...
class HILAL:
    def __init__(self, config: HILALConfig):
        self.config = config
    ...
    def run(self):
        ...
        results_path = os.path.splitext(self.updated_checkpoint_path)[0] + "TOPEIGEN_DIV_PARAMS_results.json"
    ...
    def _compute_descriptors(self):
        ...
        for layer_id in self.layer_ids:
            ...
            for batch in batches:
                ...
                lam_est = self._power_iteration_top_eig(params, grads1, self.config.lanczos_steps) / float(n_params)
    ...
    def _cluster_layers(self):
        """Run k-means (k=2) over 1D descriptors [lambda_max] to split sensitive vs robust."""
        if len(self.descriptors) == 0:
            raise RuntimeError("Descriptors not computed.")

        ids = sorted(self.descriptors.keys())
        # Use only lambda_max for clustering
        X = np.array(
            [[self.descriptors[i]["lambda_max"]] for i in ids],
            dtype=np.float64
        )
        ...
        # Favor robust cluster with low lambda_max (smaller top eigenvalue means less sensitive to noise)
        # The cluster with the smaller lambda_max centroid is the robust one
        ...
        # Order robust: prefer low lambda_max only
        self.robust_ids_ordered = sorted(
            robust_ids,
            key=lambda lid: self.descriptors[lid]["lambda_max"]
        )

        logging.info(f"Sensitive ids: {self.sensitive_ids}")
        logging.info(f"Robust ids (ordered): {self.robust_ids_ordered}")

        # Save 2D clustering plot (lambda_max vs layer index)
        try:
            base = os.path.splitext(self.config.checkpoint_path)[0]
            out_path = f"{base}_cluster_topeigen_div_params.png"

            lambda_values = np.array([self.descriptors[i]["lambda_max"] for i in ids], dtype=np.float64)
            ...
            plt.bar(layer_indices[mask_robust], lambda_values[mask_robust], 
                    color="tab:blue", label="robust", alpha=0.7, width=0.6)
            plt.bar(layer_indices[mask_sensitive], lambda_values[mask_sensitive], 
                    color="tab:red", label="sensitive", alpha=0.7, width=0.6)
            
            # Add layer IDs as labels
            for i, lid in enumerate(ids):
                plt.text(i, lambda_values[i] * 1.05, str(lid), ha='center', fontsize=8)

            plt.xlabel("Layer Index")
            plt.ylabel("lambda_max (top eigenvalue)")
            plt.title("Layer clustering based on top eigenvalue only")
