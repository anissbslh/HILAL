...
class HILALConfig:
    ...
    patience: int = 5              # kept for compatibility (unused)
    t_eval: float = 86400
...
class HILAL:
    ...
    def run(self):
        ...
        patience_window = patience  # Convergence patience window
        ...
        results_path = os.path.splitext(self.updated_checkpoint_path)[0] + "BASE_MODE_results.json"
    ...
    def _cluster_layers(self):
        """Run k-means (k=2) over 2D descriptors [ELI_norm, r] to split sensitive vs robust."""
        if len(self.descriptors) == 0:
            raise RuntimeError("Descriptors not computed.")

        ids = sorted(self.descriptors.keys())
        # Use only ELI_norm and r for clustering (no mac_ratio)
        X = np.array(
            [[self.descriptors[i]["eli_norm"], self.descriptors[i]["r"]] for i in ids],
            dtype=np.float64
        )
        ...
        # Order robust: prefer low ELI_norm, then low r
        self.robust_ids_ordered = sorted(
            robust_ids,
            key=lambda lid: (self.descriptors[lid]["eli_norm"], self.descriptors[lid]["r"])
        )

        logging.info(f"Sensitive ids: {self.sensitive_ids}")
        logging.info(f"Robust ids (ordered): {self.robust_ids_ordered}")

        # Save 2D clustering plot (ELI_norm vs r)
        try:
            base = os.path.splitext(self.config.checkpoint_path)[0]
            out_path = f"{base}_cluster_ELI_r.png"

            X2 = np.array([[self.descriptors[i]["eli_norm"], self.descriptors[i]["r"]] for i in ids], dtype=np.float64)
            
            # Use fixed marker size instead of scaling by mac_ratio
            marker_size = 100
            ...
            plt.scatter(X2[mask_robust, 0], X2[mask_robust, 1], c="tab:blue", s=marker_size, label="robust", alpha=0.8)
            plt.scatter(X2[mask_sensitive, 0], X2[mask_sensitive, 1], c="tab:red", s=marker_size, label="sensitive", alpha=0.8)
            for i, lid in enumerate(ids):
                plt.annotate(str(lid), (X2[i, 0], X2[i, 1]), fontsize=6, alpha=0.9)

            plt.xlabel("ELI_norm")
            plt.ylabel("r = lambda_max / trace")
            plt.title("Layer clustering")
